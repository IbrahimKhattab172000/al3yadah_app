...
class PatientDetailsView extends StatelessWidget {
    ...
    sessionJson.forEach((key, value) {
      if (value is Map) {
        ...
        value.forEach((nestedKey, nestedValue) {
          detailWidgets.add(
              AppDataShowCard(title: "$nestedKey ($key)", value: nestedValue));
        });
      } else {
        // For direct, non-Map values
        detailWidgets.add(AppDataShowCard(title: key, value: value));
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    Map<String, dynamic> sessionJson = patient.sessions[index].toJson();
